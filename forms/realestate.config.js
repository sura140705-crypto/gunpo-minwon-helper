...
var FORM={
  ...
  /* 인쇄 후 서명·날인해야 하는 칸 — 이름이 채워졌을 때만 형광 */
  signatureHI:function(v){
    var HI=[];
    if(v.sign_seller) HI.push([305,729.4,462,740.4]);
    if(v.sign_buyer)  HI.push([305,739.5,462,750.5]);
    if(v.sign_agent)  HI.push([343,749.5,462,760.5]);
    return HI;
  },
  ...
  STEPS:[
    ...
    {n:5, short:"중개사", title:"개업공인중개사 (중개 시)",
      ...
      body:function(A){ var h='';
        ...
        h+=A.inputHtml({k:"a_name", label:"성명(법인명)", ph:"박중개"});
        ...
        h+=A.inputHtml({k:"a_phone", label:"전화번호", type:"phone", half:true});
    ...
    {n:7, short:"소재지·면적", title:"⑥⑦ 어디에 있는 부동산인가요?",
      ...
      why:"소재지는 지번까지(아파트 등은 동·호수까지) 적습니다. 면적은 토지대장의 토지면적, 건축물대장의 건축물 면적(아파트 등 집합건축물은 전용면적)을 적습니다.",
      kind:"parcel",
      required:function(s){ return String(s.p_addr||"").trim()?[]:["소재지"]; },
      body:function(A){ var h='';
        h+=A.inputHtml({k:"p_addr", label:"⑥ 소재지", req:true, ph:"경기도 군포시 산본로 000, 101동 1001호"});
        h+=A.inputHtml({k:"p_jimok", label:"지목", half:true, ph:"대",
          help:"토지대장에 적힌 지목(대, 전, 답 등)."});
        h+=A.inputHtml({k:"p_landArea", label:"토지면적(㎡)", half:true, ph:"52.43"});
        ...
        h+='<div class="q-help" style="margin-top:8px">대지권비율 — 아파트 등 집합건축물의 등기사항증명서에 적힌 비율. 예) 45000분의 52</div>';
        h+=A.inputHtml({k:"p_daejiDen", label:"대지권비율 ( ○○ 분의", half:true});
        h+=A.inputHtml({k:"p_daejiNum", label:"○○ )", half:true});
  ...
  applySample:function(state, kind){
    ...
    Object.assign(state,{
      ...
      a_addr:"경기도 군포시 산본로 111, 1층", a_mobile:"01055556666",
      d_kind:"토지 및 건축물", d_bldgKind:"아파트",
      p_addr:"경기도 군포시 산본로 000, 101동 1001호",
      p_jimok:"대", p_landArea:"52.43", p_bldgArea:"84.97",
      p_daejiDen:"45000", p_daejiNum:"52",
    ...
    if(kind==="right"){
      ...
      Object.assign(state,{
        ...
        p_addr:"경기도 군포시 금정동 000 재건축 조합 101동 1502호",
        p_jimok:"대", p_landArea:"48.10", p_bldgArea:"74.52",
        p_daejiDen:"38000", p_daejiNum:"48",
        ...
        v_addr:"경기도 군포시 금정동 000-0", v_jimok:"대",
        v_landArea:"66.20", v_bldgArea:"59.80",
        v_daejiDen:"38000", v_daejiNum:"66",

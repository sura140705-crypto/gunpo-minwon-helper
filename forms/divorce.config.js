...
var FORM={
  ...
  /* 안내 기둥의 준비물 — **경로에 따라 갈린다.**
     ⛔ 혼인의 목록을 그대로 옮기지 마라. 이혼은 확인을 받은 뒤 부부 중 **한 사람만 와도**
        신고할 수 있으므로 「신분증 두 장」이 아니다.
     ⛔ 이혼 종류를 고른 뒤에는 **그 갈래의 서류만** 낸다 — 둘을 함께 보여주면
        시민이 없는 서류를 떼러 간다.
     ⛔ 친권자 지정 관련 서류는 여기에 넣지 않는다(업무규칙을 추정하지 않는다). */
  ready:function(s){
    var a=[
    ...
    if(s && s.divType==="협의이혼"){
      a.push({ t:"협의이혼의사확인서 등본", s:"법원에서 받은 것", g:"cert", req:true });
    }else if(isJudicial(s)){
      a.push({ t:"판결 등본 및 확정증명서", s:"조정·화해로 이혼한 경우에는 그 재판서류", g:"cert", req:true });
    }else{
      /* 아직 고르기 전 — 허브(Main)와 같은 문법으로 두 갈래를 **짧게** 적어 둔다. */
      a.push({ t:"협의이혼의사확인서 등본", s:"협의이혼일 때 · 법원에서 받은 것", g:"cert" });
      a.push({ t:"판결 등본 및 확정증명서", s:"재판이혼일 때 · 조정·화해면 그 재판서류", g:"cert" });
    }
    return a;
  },
  ...
  STEPS:[
      ...
      body:function(A){
        ...
        if(A.state.divType==="협의이혼"){
          ...
          h+='<div class="note-box"><b>협의이혼</b>은 <b>법원의 협의이혼의사확인</b>을 받은 뒤에 신고합니다. '
            +'접수할 때 <b>협의이혼의사확인서 등본</b>이 필요하며, ④ 재판확정일자·법원명은 <b>비워 둡니다.</b></div>';
        }
        if(isJudicial(A.state)){
          h+='<div class="note-box"><b>재판상 이혼</b>(판결·조정 등)은 재판이 확정된 날과 법원 이름을 적습니다. '
            +'접수할 때 <b>판결 등본 및 확정증명서</b>가 필요하며, 조정·화해로 이혼한 경우에는 <b>그 재판서류</b>를 가져오세요.</div>';
    ...
    {n:10, short:"완료", title:"작성 내용 확인", q:"입력한 내용을 확인하세요.", kind:"summary",
      body:function(){
        var extra = (state.divType==="협의이혼")
          ? '협의이혼은 <b>법원의 협의이혼의사확인서 등본</b>이 필요합니다. '
          : '재판상 이혼은 <b>판결 등본 및 확정증명서</b>(조정·화해로 이혼한 경우에는 그 재판서류)가 필요합니다. ';
        return buildSummary()
          +'<div class="info-box">인쇄한 뒤, 서명 또는 날인을 직접 하고 민원실에 제출하세요. '
          +extra+'그 밖의 첨부서류는 담당 직원이 안내합니다.</div>';
      }}
  ],

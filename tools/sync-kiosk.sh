...
# 배포 HTML 동기화 도구
#
# 루트(원본) → kiosk-app/app/ 로 배포 파일(서식 helper + index.html)을 복사한다.
# 서식을 수정하면 루트 파일만 고치고 이 스크립트를 돌리면 배포 위치가 맞춰진다.
#
# 사용법:
#   bash tools/sync-kiosk.sh          # 루트 → 배포 위치로 복사(동기화)
#   bash tools/sync-kiosk.sh --check  # 복사하지 않고 동일한지 검증만(다르면 exit 1)
#
...
# 동기화 대상 배포 디렉터리
# 2026.08 크롬 방식(키오스크배포/)을 폐지해 배포처는 한 곳이다 — 보안성 검토 대응(CHANGELOG 참조).
DESTS=(
  "kiosk-app/app"
)

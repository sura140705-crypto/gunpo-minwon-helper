...
`engine/base.html` 의 `<!--SITE-CONFIG v1-->` … `<!--/SITE-CONFIG-->` 블록이 **원본**이고,
엔진 5종은 재빌드로, 손작성 3종과 허브는 손으로 같은 내용을 받는다(AGENTS.md §1).
...
드리프트만 보므로 이 어긋남을 잡지 못한다.
    ...
    python tools/check-site-block.py --fix     # base.html 내용으로 손작성 3종·허브를 맞춘다
...
def main():
    ...
    want, _ = block_of(SRC)
    if not want:
        print('X engine/base.html 에 SITE-CONFIG 블록이 없습니다')
        return 1
    ...
    for name in HAND + BUILT:
        ...
        if fix and name in HAND:
            ...
            print('  %-28s → base.html 내용으로 맞췄습니다' % name)
            bad -= 1
        elif fix:
            print('  %-28s → `node tools/build-form.js` 로 재빌드하세요' % name)

    if bad:
        print('\n%d 곳이 어긋납니다. `--fix` 로 손작성 3종·허브를 맞추고, 엔진 5종은 재빌드하세요.' % bad)
        return 1
    print('\n전부 같습니다 — 9개 화면')
    return 0

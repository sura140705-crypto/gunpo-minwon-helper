...
> `engine/base.html` 도 함께 고치므로 **엔진 서식 5종은 재빌드**해야 반영된다.
...
현재 엔진 생성본 = 출생·사망·개명·증명서·부동산거래계약 **5종**. 손작성본은 여권·혼인·이혼 3종이며,
엔진 공용 스니펫(유휴 초기화·인쇄 안내 모달 등)을 고칠 때는 이 3종도 같은 내용으로 함께 손봐야 합니다.

...
한 단계에 입력칸이 스물한 개 쌓여도 어느 도구도 말하지 않는다(2026.09.23 증명서). 이 도구가
각 단계에서 **시민이 굴려야 하는 px**을 잰다. ⛔ **합격·불합격을 가르지 않는다**(언제나 0).
세로 넘침은 굴리면 읽히므로 그 자체가 결함이 아니다 — 어디까지 허용할지는 업무·디자인 판단이고
이 도구는 근거만 준다. 그래서 위의 **필수 통과 목록에는 없다.**
...
> ### ✅ 8종 이식 완료 — 여기가 기준점이다 (커밋 `d4c5960` · 2026.08.29)
>
...
> 서식 하나에서 색·아이콘·모션·Review 를 손대면 다시 갈라진다.
>
> | 공통 마감 단계 | 무엇 |
> |---|---|
> | ① **실사용 QA** | 키오스크에서 8종을 실제로 돌려 본다(3분 유휴·인쇄 후 초기화·지정 프린터 포함) |
> | ② **Review / 인쇄 미리보기 단순화** | 마지막 확인 화면과 미리보기의 구조를 8종 공통으로 |
> | ③ **Color & Visual Polish** | BAR tint · primary/secondary 위계 · 미리보기 강조도 · 색 농도 |
> | ④ **Icon Polish** | 목록·RAIL·준비물 아이콘 한 벌 |
> | ⑤ **Motion** | 전환·눌림 반응 |
>
> 📌 이 단계들 전에는 **개별 서식 기능을 추가 분석하거나 리팩터링하지 않는다.**
> 비차단 잔재를 발견해도 **그 자리에서 고치지 말고 `CHANGELOG.md` 의 TODO 로만** 남긴다.
> ⚠️ 예외는 그대로다 — **실제 구현 결함**(값이 인쇄에 안 실림 · 필수 우회 · 조건 변경 뒤 잔상 ·
> JS 오류 · 인쇄 좌표/쪽수 · 화면 넘침 · 개인정보 초기화 · 다른 서식 회귀)은 발견 즉시 고친다.
